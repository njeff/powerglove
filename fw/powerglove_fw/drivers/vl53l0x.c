...
uint16_t VL53L0X_getTimeout() { return io_timeout; }
...
uint8_t VL53L0X_getAddress() { return address; }
...
static const nrf_twi_mngr_t * i2c_dev;

bool VL53L0X_init(bool io_2v8, const nrf_twi_mngr_t* i2c)
{
  i2c_dev = i2c;
  io_timeout = 0;
  did_timeout = false;
...
static void i2c_reg_write2(uint8_t i2c_addr, uint8_t reg_addr, uint16_t data) {
...
static void i2c_reg_write4(uint8_t i2c_addr, uint8_t reg_addr, uint32_t data) {
  uint8_t buf[5] = {reg_addr, 
    (data >> 24) & 0xFF,
    (data >> 16) & 0xFF,
    (data >> 8)  & 0xFF, 
     data        & 0xFF};
  nrf_twi_mngr_transfer_t const write_transfer[] = {
    NRF_TWI_MNGR_WRITE(i2c_addr, buf, 5, 0),
  };
  ret_code_t error_code = nrf_twi_mngr_perform(i2c_dev, NULL, write_transfer, 1, NULL);
  APP_ERROR_CHECK(error_code);
}
...
static void writeReg(uint8_t reg, uint8_t value)
{
  ...
  */
  i2c_reg_write(address, reg, value);
}
...
static void writeReg16Bit(uint8_t reg, uint16_t value)
{
  ...
  last_status = bus->endTransmission();
  */
  i2c_reg_write2(address, reg, value);
}
...
static void writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  last_status = bus->endTransmission();
  */
  i2c_reg_write4(address, reg, value);
}
...
static uint8_t readReg(uint8_t reg)
{
  ...
  value = bus->read();
  */
  value = i2c_reg_read(address, reg); 
  return value;
...
static uint16_t readReg16Bit(uint8_t reg)
{
  ...
  */
  uint8_t d[2];
  i2c_read_bytes(address, reg, d, 2);
...
static uint32_t readReg32Bit(uint8_t reg)
{
  ...
  */
  uint8_t d[4];
  i2c_read_bytes(address, reg, d, 4);
  value = ((uint32_t)(d[0]) << 24) | ((uint32_t)(d[1]) << 16);
  value |= ((uint32_t)(d[2]) << 8) | d[3];
  return value;
...
static void readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  ...
  */
  i2c_read_bytes(address, reg, dst, count);
}
